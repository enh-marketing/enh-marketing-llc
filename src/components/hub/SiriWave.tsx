 ...
 *
 * @40973894/siri-wave, kept exactly as delivered. Only this header and the `cn`
 * import path are changed: it ships pointing at `@/lib/utils` and this project
 * keeps that helper at `@/lib/cn`.
 *
 * WHY THIS ONE, after three others were tried and dropped. It is dependency
 * free: one fullscreen triangle and a fragment shader on a raw WebGL context,
 * which is the same shape as hub/BlackHoleHeroSection and costs nothing to add.
 * The gradient orb that was tried before it needed @react-three/fiber, and
 * measured, that took this page from 186 KB of JavaScript to 401 KB gzipped.
 *
 * AND IT IS THE ONLY ONE THAT CHECKS ITS OWN SHADERS. It reads COMPILE_STATUS
 * and throws with the info log. The orb-shader tried before it did not, so when
 * something went wrong it drew black and said nothing, which cost an hour.
 *
 * TWO THINGS THAT MATTER TO THE CALLER.
 *
 * It is a FIXED SQUARE. `size` is CSS pixels and the canvas is size by size,
 * with the dimensions written inline. It does not fill its container and it
 * does not read its own box, so it is immune to the trap that broke the
 * gradient orb: ancestor transforms cannot feed back into its resolution the
 * way they did through react-three-fiber's getBoundingClientRect. A caller may
 * scale and translate it freely. What a caller must not do is drive `size` from
 * anything continuous: it sits in the effect's dependency array, so every
 * change recompiles both shaders.
 *
 * And `renderScale` is the lever for weak devices: the canvas is drawn at
 * size * renderScale and stretched up by CSS, so halving it quarters the
 * fragment work.
 *
 * gl_FragColor writes alpha 1, so the canvas is opaque, the same as every other
 * scene here. That is why the page's star field is screened over the scenes
 * rather than placed behind them; see hub/Journey.tsx. */
