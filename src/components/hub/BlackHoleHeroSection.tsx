/* eslint-disable react-hooks/refs, @typescript-eslint/no-explicit-any, prefer-const --
 * Four of this file's own habits trip our rules, and none of them is ours to
 * change: it is imported as delivered, and editing it to satisfy a linter is
 * how an import quietly becomes a fork.
 *
 *   react-hooks/refs        it writes its props into a ref during render. That
 *                           is the pattern that makes the camera free to move,
 *                           and OrbitalHeroSection is scoped off for the same
 *                           line for the same reason.
 *   no-explicit-any         two event handlers.
 *   prefer-const            three locals in the ray walk.
 *
 * If this is ever genuinely forked, delete this block and fix them properly.
 */

/* IMPORTED FROM 21ST.DEV, UNCHANGED.
 *
 * @yura/blackhole-hero-section, kept exactly as delivered. It needs nothing:
 * `registryDependencies` and `npmDependencies` are both empty, it imports only
 * React, and it already does the things this page would otherwise have had to
 * add. It pauses on `visibilitychange`, it pauses itself through an
 * IntersectionObserver when it is off screen, it honours
 * prefers-reduced-motion by holding a still frame, and it carries `resolution`,
 * `steps` and `maxDpr` so the cost can be dialled down without touching the
 * shader.
 *
 * It is also the same author as OrbitalHeroSection, and shares its shape: props
 * are held in a ref that the animation loop reads each frame, so moving the
 * camera never rebuilds the scene. That is what lets the chapter drive it from
 * scroll for nothing. See hub/chapters/Horizon.tsx.
 */
