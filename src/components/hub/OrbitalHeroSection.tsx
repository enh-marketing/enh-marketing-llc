...
/* -------------------------------------------------------------------------- */
/*  ORBITAL — @yura/orbital-hero-section on 21st.dev, IMPORTED UNCHANGED.     */
/*                                                                            */
/*  Nothing in this file is adapted. It arrived with no dependencies, in      */
/*  TypeScript, already sized to its own block with `h-full w-full`, already  */
/*  taking children, already handling devicePixelRatio, prefers-reduced-      */
/*  motion, IntersectionObserver, visibilitychange, ResizeObserver and full   */
/*  teardown. It is the only one of the three that needed no changes at all.  */
/*                                                                            */
...
export interface OrbitalHeroSectionProps extends React.HTMLAttributes<HTMLDivElement> {
...
export function OrbitalHeroSection({
  ...
  /* Upstream's own pattern, kept: the animation loop reads the latest props
     through a ref so that changing one does not tear down and rebuild the
     scene. The rule objects to writing a ref during render; the component is
     imported as it is, so the rule is scoped off here rather than the
     behaviour changed. */
