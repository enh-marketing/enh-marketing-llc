...
/*                                                                            */
/*  IMPORTED FROM @yura/airlock-spaceship-hero ON 21ST.DEV, AS IT IS. The     */
/*  component's markup, palettes, props, timings, easing, copy defaults and   */
/*  behaviour are untouched. Two things differ, and both are required for it  */
/*  to work in this project rather than preferences:                          */
/*                                                                            */
/*    1. `cn` comes from @/lib/cn. This project has no @/lib/utils.           */
/*    2. Lenis is stopped while the hero holds the wheel and started again    */
/*       when it hands the page back. This site runs one Lenis instance in    */
/*       fx/SmoothScroll.tsx which also listens for wheel and drives window   */
/*       scroll, so without this two scroll controllers fight over every      */
/*       gesture, and pinning the body to position:fixed leaves Lenis         */
/*       tracking a position that no longer exists.                           */
/*                                                                            */
/*    3. `title` and `tagline` take a node rather than a string. Nothing      */
/*       about how they are drawn, timed, blurred or faded changes; the       */
/*       slots simply hold whatever is put in them. This page's headline is   */
/*       set in three weights with its last word turning over on a clock,     */
/*       and a string cannot carry that. Widening a slot is not rewriting     */
/*       the component: pass a string and it behaves as published.            */
/*                                                                            */
/*  THE VIDEO AND POSTER COME FROM OUR OWN ORIGIN. The component's defaults   */
/*  point at jsDelivr, out of the author's GitHub repo: not ours, and a hero  */
/*  that cannot paint when someone else's CDN is having a bad day is not a    */
/*  hero. hub/Opening passes /hero/airlock.mp4 and /hero/airlock-poster.jpg   */
/*  instead. The defaults stay so the file still stands on its own.           */
/*                                                                            */
...
export default function AirlockHero({
  ...
  useEffect(() => {
    ...
    function engageLock() {
      ...
      // This site's own smooth scrolling has to stand down while the hero
      // holds the wheel, or both are steering.
      getLenis()?.stop();
    ...
    /* AND CALL IT ONCE BY HAND IF THE FILM IS ALREADY THERE. `loadeddata` is a
       one-shot event, not a state: a file that is in the cache, or that a
       browser decided to preload eagerly, has fired it before this effect ever
       ran, and the listener then waits for an event that will never come
       again. `ready` stays false, so the video is left at opacity 0, and
       `duration` stays 0, so nothing seeks. It shows as a hero that is a black
       rectangle no matter how far you scroll, and it happens on every reload
       once the file is warm. */
