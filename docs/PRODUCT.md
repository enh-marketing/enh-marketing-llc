...
**The one exception, and its boundary.** The AI Hub pillar page at `/ai-hub` has no client document and never had one, which is why it quotes: every category title is the label `src/lib/sitemap.ts` already gives that page and every body is that service's own `meta.description`, with `npm run check:copy` failing the build if either drifts. On 2026-09-09 Divij authorised copy to be written for that page specifically, having been shown this rule and that no document exists for it. The seven quoted category blocks stay quoted, on his instruction, and the gate still enforces them. What is written is the connective layer only: the head tags, the structured data, and any framing the story needs. It is written to the same discipline as the documents, which is the whole reason it is allowed: no metric, no percentage, no currency, no date and no guarantee, because the source documents carry none. This exception is for this one page. Everything under `/ai-hub/*` is still verbatim.
